...
How does it work?
The plugin scrapes data from the web interface, in order to understand the id of the stage. It then calls the maestro API to download the fixture list, and searches for fixtures with a shutter and a color wheel or a dedicated Strobe channel, which also contain the attributes required in point 4 above in the name. It also forcefully ignores any fixture with the word IGNORE in the name. 
